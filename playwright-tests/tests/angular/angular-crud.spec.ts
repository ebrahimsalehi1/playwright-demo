...
test.describe("Angular App - CRUD Operations", () => {
  ...
  test("should create a new user (POST)", async ({ page }) => {
    // Fill in the form
    await page.locator('[data-testid="input-name"]').fill("Angular Test User");
    await page
      .locator('[data-testid="input-email"]')
      .fill("angular@example.com");
    await page.locator('[data-testid="input-role"]').fill("Angular Developer");
    ...
    const newUser = page.locator('.user-card:has-text("Angular Test User")');
    await expect(newUser).toBeVisible();
    await expect(
      newUser.locator('p:has-text("angular@example.com")')
    ).toBeVisible();
  });
  ...
  test("should edit an existing user (PUT)", async ({ page }) => {
    ...
    await nameInput.fill("John Angular Updated");
    ...
    await expect(
      updatedUser.locator('h4:has-text("John Angular Updated")')
    ).toBeVisible();
  });
